...
class StartupStatusControllerIntegrationTest {
  ...
  @MockBean private StartupReadinessService startupReadinessService;

  @Test
  void returnsUnauthorizedWithoutAuthentication() throws Exception {
    mockMvc.perform(get("/api/v1/system/startup-status")).andExpect(status().isUnauthorized());
  }
  ...
  void returnsCurrentStartupStatusForAuthenticatedUsers() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/system/startup-status").with(authenticatedUser()))
