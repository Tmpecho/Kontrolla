...
class DeviationControllerIntegrationTest {
  ...
  void managerCanCreateManageAndFetchDeviationTimeline() throws Exception {
    ...
    String createResponse =
        mockMvc
            .perform(
                post("/api/v1/organizations/%s/establishments/%s/deviations"
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/%s/establishments/%s/deviations/%s/assignment"
                    .formatted(organization.getId(), establishment.getId(), deviationId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/%s/establishments/%s/deviations/%s/status"
                    .formatted(organization.getId(), establishment.getId(), deviationId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/%s/establishments/%s/deviations/%s"
                    .formatted(organization.getId(), establishment.getId(), deviationId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/establishments/%s/deviations/%s/timeline"
                    .formatted(organization.getId(), establishment.getId(), deviationId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(
            jsonPath("$.timeline[4].note")
                .value("Thermostat recalibrated and follow-up temperature check passed."));

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/deviations/%s"
                    .formatted(organization.getId(), establishment.getId(), deviationId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/deviations"
                    .formatted(organization.getId(), establishment.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/deviations".formatted(organization.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items.length()").value(1))
  ...
  void listEndpointsReturnMostRecentDeviationsFirstByDefault() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/deviations?size=1"
                    .formatted(organization.getId(), establishment.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items.length()").value(1))
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/deviations?size=1".formatted(organization.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items.length()").value(1))
        .andExpect(jsonPath("$.items[0].title").value("Newest deviation"));
  ...
  void listDeviationEndpointsCapRequestedPageSize() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/deviations?size=500"
                    .formatted(organization.getId(), establishment.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/deviations?size=500".formatted(organization.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.size").value(100));
  }
  ...
  void memberFromAnotherOrganizationCannotReadDeviationDetails() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/deviations/%s"
                    .formatted(organizationA.getId(), establishment.getId(), deviationId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + outsiderToken))
        .andExpect(status().isForbidden());

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/deviations".formatted(organizationA.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + outsiderToken))
        .andExpect(status().isForbidden());
  }
