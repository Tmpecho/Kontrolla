...
class MembershipAuditIntegrationTest {
  ...
  void membershipCreationPersistsAuditEvent() throws Exception {
    ...
    String establishmentId =
        createEstablishment(adminToken, organizationId, "Restaurant", "RESTAURANT");
    String employeeId =
        userRepository
            .findByEmailIgnoreCase("employee@example.com")
            .orElseThrow()
            .getId()
            .toString();
    ...
    List<AuditEvent> matchingEvents =
        auditEventRepository.findAll().stream()
            ...
            .filter(
                auditEvent ->
                    membershipId.equals(
                        readJson(auditEvent.getMetadataJson()).path("membershipId").asText()))
            .toList();
  ...
  private String createEstablishment(
      String adminToken, String organizationId, String name, String type) throws Exception {
    String response =
        mockMvc
            .perform(
