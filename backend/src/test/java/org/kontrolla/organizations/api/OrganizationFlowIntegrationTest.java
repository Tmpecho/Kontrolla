...
class OrganizationFlowIntegrationTest {
  ...
  void platformAdminCanCreateOrganizationAndManagerCanCreateEstablishment() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s".formatted(organizationId))
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/establishments".formatted(organizationId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + managerToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void tenantMemberCannotReadAnotherOrganizationsData() throws Exception {
    ...
    String establishmentResponse =
        mockMvc
            .perform(
                post("/api/v1/organizations/%s/establishments".formatted(orgBId))
                    .with(csrf())
                    .header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s".formatted(orgBId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAToken))
        .andExpect(status().isForbidden());

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members".formatted(orgBId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAToken))
        .andExpect(status().isForbidden());

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments".formatted(orgBId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAToken))
        .andExpect(status().isForbidden());

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s".formatted(orgBId, establishmentId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAToken))
        .andExpect(status().isForbidden());
  }
  ...
  void organizationMemberCanListOwnOrganizationsMembers() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members".formatted(organizationId))
  ...
  void memberListingHidesInactiveMembershipsByDefaultButCanIncludeThem() throws Exception {
    ...
    String inactiveMembershipResponse =
        mockMvc
            .perform(
                post("/api/v1/organizations/%s/members".formatted(organizationId))
                    .with(csrf())
    ...
    String inactiveMembershipId =
        objectMapper.readTree(inactiveMembershipResponse).get("id").asText();
            ...
            jsonPath("$.items[*].id")
                .value(
                    org.hamcrest.Matchers.not(
                        org.hamcrest.Matchers.hasItem(inactiveMembershipId))));

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members?includeInactive=true".formatted(organizationId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAdminToken))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.items[*].id").value(org.hamcrest.Matchers.hasItem(inactiveMembershipId)));
  }
  ...
  void memberListingCanBeFilteredByEstablishmentScope() throws Exception {
    createUser("admin@example.com", "Admin", "User", Set.of(GlobalRole.PLATFORM_ADMIN));
    User orgAdmin = createUser("orgadmin@example.com", "Org", "Admin", Set.of());
    User restaurantEmployee =
        createUser("restaurant@example.com", "Restaurant", "Employee", Set.of());
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members?establishmentId=%s"
                    .formatted(organizationId, restaurantId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAdminToken))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.items[*].userEmail")
                .value(org.hamcrest.Matchers.hasItem("orgadmin@example.com")))
        .andExpect(
            jsonPath("$.items[*].userEmail")
                .value(org.hamcrest.Matchers.hasItem("restaurant@example.com")))
        .andExpect(
            jsonPath("$.items[*].userEmail")
                .value(
                    org.hamcrest.Matchers.not(org.hamcrest.Matchers.hasItem("bar@example.com"))));

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members?establishmentId=%s"
                    .formatted(organizationId, barId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAdminToken))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.items[*].userEmail")
        ...
        .andExpect(
            jsonPath("$.items[*].userEmail")
                .value(org.hamcrest.Matchers.hasItem("bar@example.com")))
        .andExpect(
            jsonPath("$.items[*].userEmail")
                .value(
                    org.hamcrest.Matchers.not(
                        org.hamcrest.Matchers.hasItem("restaurant@example.com"))));
  }
  ...
  void memberListingRejectsForeignEstablishmentId() throws Exception {
    ...
    String foreignEstablishmentResponse =
        mockMvc
            .perform(
                post("/api/v1/organizations/%s/establishments".formatted(orgBId))
                    .with(csrf())
    ...
    String foreignEstablishmentId =
        objectMapper.readTree(foreignEstablishmentResponse).get("id").asText();
    String orgAdminToken = login("orgadmin@example.com", "password123");

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members?establishmentId=%s"
                    .formatted(orgAId, foreignEstablishmentId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAdminToken))
  ...
  void membershipUpdatePreservesScopedEstablishmentAccessWhenScopeIsOmitted() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/v1/organizations/%s/members/%s".formatted(organizationId, membershipId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAdminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void tenantMemberCannotModifyAnotherOrganizationsData() throws Exception {
    ...
    String orgBMembershipResponse =
        addMembership(adminToken, orgBId, orgBUser.getId(), "ORG_EMPLOYEE");
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/members".formatted(orgBId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
    ...
    mockMvc
        .perform(
            patch("/api/v1/organizations/%s/members/%s".formatted(orgBId, orgBMembershipId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + orgAToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void userWithoutMembershipCannotAccessOrganizationData() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments".formatted(organizationId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + noMemberToken))
        .andExpect(status().isForbidden());
  }
  ...
  void nonPlatformAdminCannotAccessAdminOrganizationsOrUsersEndpoints() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/admin/users").header(HttpHeaders.AUTHORIZATION, "Bearer " + managerToken))
        .andExpect(status().isForbidden());
  ...
  void organizationEmployeeCannotCreateEstablishmentInOwnOrganization() throws Exception {
    createUser(
        "admin-establishments@example.com",
        "Admin",
        "Establishments",
        Set.of(GlobalRole.PLATFORM_ADMIN));
    User employee =
        createUser("employee-establishments@example.com", "Employee", "Establishments", Set.of());
        ...
        .perform(
            patch(
                    "/api/v1/organizations/%s/members/%s"
                        .formatted(organizationId, managerMembershipId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + managerToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void membershipListCapsRequestedPageSize() throws Exception {
    User admin =
        createUser("admin-cap@example.com", "Admin", "Cap", Set.of(GlobalRole.PLATFORM_ADMIN));
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/members?size=500".formatted(organizationId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + managerToken))
        .andExpect(status().isOk())
  ...
  private User createUser(String email, String firstName, String lastName, Set<GlobalRole> roles) {
    User user =
        new User(email, firstName, lastName, passwordEncoder.encode("password123"), true, roles);
  ...
  private String addMembership(String adminToken, String organizationId, Object userId, String role)
      throws Exception {
    return mockMvc
        .perform(
            post("/api/v1/organizations/%s/members".formatted(organizationId))
