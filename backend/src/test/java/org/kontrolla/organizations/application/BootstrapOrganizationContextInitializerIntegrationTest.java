...
class BootstrapOrganizationContextInitializerIntegrationTest {
  ...
  @Autowired private EstablishmentServingHoursRepository establishmentServingHoursRepository;
  ...
  void bootstrapDevelopmentTeamIsCreatedWithExpectedRoles() {
    ...
    User employeeOne =
        userRepository.findByEmailIgnoreCase("emma.larsen@example.com").orElseThrow();
    ...
    Organization configuredOrganization =
        organizationRepository.findByNameIgnoreCase("Kontrolla Dev Org").orElseThrow();
    var restaurant =
        establishmentRepository
            .findFirstByOrganizationIdAndNameIgnoreCase(
                configuredOrganization.getId(), "Kontrolla Demo Restaurant")
            .orElseThrow();
    var bar =
        establishmentRepository
            .findFirstByOrganizationIdAndNameIgnoreCase(
                configuredOrganization.getId(), "Kontrolla Demo Bar")
            .orElseThrow();

    assertThat(
            organizationMembershipRepository.findByOrganizationIdAndUserId(
                configuredOrganization.getId(), admin.getId()))
        .get()
        .satisfies(
            membership -> {
    ...
    assertThat(
            organizationMembershipRepository.findByOrganizationIdAndUserId(
                configuredOrganization.getId(), manager.getId()))
        .get()
        .satisfies(
            membership -> {
    ...
    assertThat(
            organizationMembershipRepository.findByOrganizationIdAndUserId(
                configuredOrganization.getId(), employeeOne.getId()))
        .get()
        .satisfies(
            membership -> {
    ...
    assertThat(
            organizationMembershipRepository.findByOrganizationIdAndUserId(
                configuredOrganization.getId(), employeeTwo.getId()))
        .get()
        .satisfies(
            membership -> {
              assertThat(membership.getRole()).isEqualTo(OrganizationRole.ORG_EMPLOYEE);
    ...
    assertThat(
            organizationMembershipRepository
                .findByOrganizationIdAndAccessibleEstablishmentId(
                    configuredOrganization.getId(),
                    restaurant.getId(),
                    org.springframework.data.domain.PageRequest.of(0, 50))
                .getTotalElements())
    ...
    assertThat(
            checklistDefinitionRepository.findByEstablishmentIdAndStatus(
                restaurant.getId(), ChecklistDefinitionStatus.ACTIVE))
        .anySatisfy(
            definition -> {
    ...
    assertThat(
            checklistDefinitionRepository.findByEstablishmentIdAndStatus(
                bar.getId(), ChecklistDefinitionStatus.ACTIVE))
    ...
    assertThat(
            deviationRepository
                .findByEstablishmentIdAndOrganizationId(
                    restaurant.getId(),
                    configuredOrganization.getId(),
                    org.springframework.data.domain.PageRequest.of(0, 20))
                .getTotalElements())
    ...
    assertThat(
        ...
        .isGreaterThanOrEqualTo(5);
    assertThat(
            establishmentServingHoursRepository.findByEstablishmentIdOrderByDayOfWeekAsc(
                restaurant.getId()))
        .hasSize(7)
        .anySatisfy(
            hours -> {
    ...
    Organization curatedOrganization =
        organizationRepository.findByNameIgnoreCase("Nordic Table Group").orElseThrow();
    var curatedBar =
        establishmentRepository
            .findFirstByOrganizationIdAndNameIgnoreCase(
                curatedOrganization.getId(), "Vulkan Tasting Room")
            .orElseThrow();
    assertThat(
            organizationMembershipRepository.findByOrganizationIdAndUserId(
                curatedOrganization.getId(), manager.getId()))
        .get()
        .satisfies(
            membership -> {
    ...
    assertThat(
            organizationMembershipRepository.findByOrganizationIdAndUserId(
                curatedOrganization.getId(), employeeOne.getId()))
        .get()
        .satisfies(
            membership -> {
              assertThat(membership.getRole()).isEqualTo(OrganizationRole.ORG_EMPLOYEE);
    ...
    assertThat(
            checklistDefinitionRepository.findByEstablishmentIdAndStatus(
                curatedBar.getId(), ChecklistDefinitionStatus.ACTIVE))
        .extracting(definition -> definition.getServiceArea())
        .contains(ChecklistServiceArea.IK_MAT, ChecklistServiceArea.IK_ALKOHOL);

    Organization secondCuratedOrganization =
        organizationRepository.findByNameIgnoreCase("Fjord Service Collective").orElseThrow();
    var inactiveEstablishment =
        establishmentRepository
            .findFirstByOrganizationIdAndNameIgnoreCase(
                secondCuratedOrganization.getId(), "Festivalpaviljongen")
            .orElseThrow();
    assertThat(
            checklistDefinitionRepository.findByEstablishmentIdAndStatus(
                inactiveEstablishment.getId(), ChecklistDefinitionStatus.ACTIVE))
        .isEmpty();
    assertThat(
            deviationRepository
                .findByEstablishmentIdAndOrganizationId(
