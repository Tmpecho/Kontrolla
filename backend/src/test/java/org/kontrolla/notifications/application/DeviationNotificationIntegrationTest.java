...
class DeviationNotificationIntegrationTest {
	...
	void deviationAssignmentStatusAndNotesNotifyTheAssignedUser() {
		...
		Establishment establishment = createEstablishment(organization, "Deviation Notification Restaurant");
		...
		List<org.kontrolla.notifications.domain.Notification> notifications = notificationRepository.findAll().stream()
				.sorted(Comparator.comparing(org.kontrolla.notifications.domain.Notification::getCreatedAt))
				.toList();
