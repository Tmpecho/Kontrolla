...
class AuthControllerIntegrationTest {
  ...
  void refreshRejectsReplayOfPreviouslyUsedRefreshToken() throws Exception {
    ...
    String rotatedRefreshCookie =
        Objects.requireNonNull(refreshResult.getResponse().getCookie(REFRESH_COOKIE_NAME))
            .getValue();
  ...
  void refreshRejectsMissingRefreshCookie() throws Exception {
    mockMvc
        .perform(post("/api/v1/auth/refresh").with(csrf()))
        .andExpect(status().isUnauthorized())
  ...
  void logoutRevokesRefreshTokenAndClearsRefreshCookie() throws Exception {
    ...
    org.assertj.core.api.Assertions.assertThat(
        ...
        .get()
        .satisfies(
            token -> org.assertj.core.api.Assertions.assertThat(token.getRevokedAt()).isNotNull());
  }
    ...
    performMe("not-a-jwt")
        ...
        .andExpect(
            header()
                .string(
                    HttpHeaders.WWW_AUTHENTICATE,
                    org.hamcrest.Matchers.containsString("Malformed token")));
  }
    ...
    performMe(expiredToken)
        ...
        .andExpect(
            header()
                .string(
                    HttpHeaders.WWW_AUTHENTICATE,
                    org.hamcrest.Matchers.containsString("Jwt expired")));
  }
    ...
    performMe(tokenWithWrongSignature)
        ...
        .andExpect(
            header()
                .string(
                    HttpHeaders.WWW_AUTHENTICATE,
                    org.hamcrest.Matchers.containsString("Invalid signature")));
  }
    ...
    performMe(wrongIssuerToken)
        ...
        .andExpect(
            header()
                .string(
                    HttpHeaders.WWW_AUTHENTICATE,
                    org.hamcrest.Matchers.containsString("iss claim")));
  }
  ...
  void loginAllowsCorrectPasswordAfterLockoutWindowExpires() throws Exception {
    ...
    for (int attempt = 0; attempt < 5; attempt++) {
      performLogin("alice@example.com", "wrong-password").andExpect(status().isUnauthorized());
    }

    performLogin("alice@example.com", "password123").andExpect(status().isUnauthorized());
  ...
  void scopedMembershipUsesAccessibleEstablishmentInAppContext() throws Exception {
    ...
    establishmentRepository.saveAndFlush(
        new Establishment(
            organization,
            "Blocked Establishment",
            EstablishmentType.RESTAURANT,
            EstablishmentStatus.ACTIVE));
    Establishment accessibleEstablishment =
        establishmentRepository.saveAndFlush(
            new Establishment(
                organization,
    ...
    OrganizationMembership membership =
        new OrganizationMembership(organization, user, OrganizationRole.ORG_EMPLOYEE, true, false);
  ...
  void invitedUserCanAcceptInvitationAndThenLogIn() throws Exception {
    ...
    String inviteResponse =
        mockMvc
            .perform(
                post("/api/v1/organizations/%s/members/managed-users"
    ...
    mockMvc
        .perform(get("/api/v1/auth/invitations/%s".formatted(token)))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/v1/auth/invitations/%s/accept".formatted(token))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
        ...
        .perform(
            patch(
                    "/api/v1/organizations/%s/members/%s"
                        .formatted(organization.getId(), membershipId))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + adminAccessToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void refreshLocksOutIpAfterRepeatedFailedAttempts() throws Exception {
    ...
    MvcResult loginResult =
        performLogin("alice@example.com", "password123", "198.51.100.31")
            .andExpect(status().isOk())
  ...
  void refreshLocksOutAccountAfterRepeatedReplayAttempts() throws Exception {
    ...
    MvcResult loginResult =
        performLogin("alice@example.com", "password123", "198.51.100.70")
            .andExpect(status().isOk())
            .andExpect(cookie().exists(REFRESH_COOKIE_NAME))
            .andReturn();
    ...
    MvcResult refreshResult =
        performRefresh(initialRefreshCookie, "198.51.100.70")
            .andExpect(status().isOk())
            .andExpect(cookie().exists(REFRESH_COOKIE_NAME))
            .andReturn();
  ...
  void successfulRefreshResetsFailedAttemptCounters() throws Exception {
    ...
    MvcResult loginResult =
        performLogin("alice@example.com", "password123", "198.51.100.80")
            .andExpect(status().isOk())
            .andExpect(cookie().exists(REFRESH_COOKIE_NAME))
            .andReturn();
    ...
    String tokenB =
        Objects.requireNonNull(firstRefreshResult.getResponse().getCookie(REFRESH_COOKIE_NAME))
            .getValue();
    ...
    String tokenC =
        Objects.requireNonNull(secondRefreshResult.getResponse().getCookie(REFRESH_COOKIE_NAME))
            .getValue();
  ...
  private org.springframework.test.web.servlet.ResultActions performMe(String accessToken)
      throws Exception {
    return mockMvc.perform(
        get("/api/v1/auth/me").header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken));
  }

  private org.springframework.test.web.servlet.ResultActions performRefresh(String refreshCookie)
      throws Exception {
  ...
  private User createInvitingOrgAdmin(String email) {
    ...
    User orgAdmin =
        new User(email, "Org", "Admin", passwordEncoder.encode("password123"), true, Set.of());
  ...
  private String issueAccessToken(
      JwtEncoder encoder, UUID userId, String email, Instant issuedAt, Instant expiresAt) {
