...
class DocumentControllerIntegrationTest {
  ...
  void managerCanCreateUpdateReplaceDownloadDeleteAndListDocuments() throws Exception {
    ...
    String createResponse =
        mockMvc
            .perform(
                multipart(
                        "/api/v1/organizations/%s/establishments/%s/documents"
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/%s/establishments/%s/documents/%s"
                    .formatted(organization.getId(), establishment.getId(), documentId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
    ...
    mockMvc
        .perform(
            multipart(
                    "/api/v1/organizations/%s/establishments/%s/documents/%s/file"
                        .formatted(organization.getId(), establishment.getId(), documentId))
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/documents/%s"
                    .formatted(organization.getId(), establishment.getId(), documentId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/documents/%s/file"
                    .formatted(organization.getId(), establishment.getId(), documentId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
        ...
        .andExpect(
            header()
                .string(
                    ...
                    containsString("alcohol-service-licence-2026.pdf")))
        .andExpect(content().bytes(pdfBytes("license-v2")));

    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/documents?serviceArea=IK_ALKOHOL"
                    .formatted(organization.getId(), establishment.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            delete(
                    "/api/v1/organizations/%s/establishments/%s/documents/%s"
                        .formatted(organization.getId(), establishment.getId(), documentId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
  ...
  void outsiderFromAnotherOrganizationCannotReadDocuments() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/documents/%s"
                    .formatted(organizationA.getId(), establishment.getId(), documentId))
  ...
  void managerCanAssignMultipleAuditReadersAndAssignedReaderCanAcknowledge() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/establishments/%s/documents/%s/acknowledge-read"
                    .formatted(organization.getId(), establishment.getId(), documentId))
  ...
  private MockMultipartFile metadataPart(
    ...
    return new MockMultipartFile(
        "metadata", "", MediaType.APPLICATION_JSON_VALUE, json.getBytes(StandardCharsets.UTF_8));
  }
