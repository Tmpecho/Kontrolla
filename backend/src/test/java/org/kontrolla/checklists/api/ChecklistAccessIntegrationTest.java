...
class ChecklistAccessIntegrationTest {
  ...
  void organizationEmployeeCannotCreateOrUpdateChecklistDefinitions() throws Exception {
    ...
    Establishment establishment =
        createEstablishment(organization, "Checklist Definition Restaurant");
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/establishments/%s/checklists/definitions"
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/%s/establishments/%s/checklists/definitions/%s"
  ...
  @Test
  void organizationEmployeeCannotAssignChecklistRunsOrFilterAnotherUsersAssignments()
      throws Exception {
    ...
    Establishment establishment =
        createEstablishment(organization, "Checklist Assignment Restaurant");
    createMembership(organization, manager, OrganizationRole.ORG_MANAGER);
    createMembership(organization, employee, OrganizationRole.ORG_EMPLOYEE);
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/establishments/%s/checklists/runs/%s/assignments"
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/checklists/runs?serviceArea=IK_MAT&assignedUserId=%s"
  ...
  void unassignedEmployeeCannotUpdateAssignedChecklistRunTask() throws Exception {
    ...
    ChecklistRun run = createRun(definition, establishment, manager);
    run.addAssignment(
        new ChecklistRunAssignment(
            assignedEmployee, manager, Instant.parse("2026-04-07T08:00:00Z")));
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/%s/establishments/%s/checklists/runs/%s/tasks/%s"
  ...
  void updatingCompletedChecklistRunTaskReturnsConflictProblemDetails() throws Exception {
    ...
    Establishment establishment =
        createEstablishment(organization, "Checklist Completed Task Restaurant");
    createMembership(organization, manager, OrganizationRole.ORG_MANAGER);
    createMembership(organization, employee, OrganizationRole.ORG_EMPLOYEE);
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message").value("Completed or cancelled checklist runs cannot be updated"));
  }
  ...
  void updatingChecklistRunTaskWithUnknownTaskIdReturnsNotFoundProblemDetails() throws Exception {
    ...
    Establishment establishment =
        createEstablishment(organization, "Checklist Missing Task Restaurant");
    createMembership(organization, manager, OrganizationRole.ORG_MANAGER);
    createMembership(organization, employee, OrganizationRole.ORG_EMPLOYEE);
  ...
  void resettingChecklistRunOutsideInProgressReturnsConflictProblemDetails() throws Exception {
    ...
    Establishment establishment =
        createEstablishment(organization, "Checklist Reset State Restaurant");
    createMembership(organization, manager, OrganizationRole.ORG_MANAGER);
    createMembership(organization, employee, OrganizationRole.ORG_EMPLOYEE);
    ...
    mockMvc
        .perform(
            post("/api/v1/organizations/%s/establishments/%s/checklists/runs/%s/reset"
                    .formatted(organization.getId(), establishment.getId(), run.getId()))
                .with(csrf())
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + employeeToken))
        .andExpect(status().isConflict())
  ...
  void userWithoutMembershipCannotReadChecklistDefinitionsOrRuns() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/%s/establishments/%s/checklists/definitions?serviceArea=IK_MAT"
                    .formatted(organization.getId(), establishment.getId()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + outsiderToken))
        .andExpect(status().isForbidden());
  ...
  private ChecklistRun createRun(
      ChecklistDefinition definition, Establishment establishment, User manager) {
    ChecklistRun run =
        new ChecklistRun(
            definition,
