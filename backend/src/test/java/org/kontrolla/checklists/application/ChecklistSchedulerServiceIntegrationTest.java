...
class ChecklistSchedulerServiceIntegrationTest {
  ...
  @Autowired private ChecklistSchedulerService checklistSchedulerService;
  ...
  void generateRunsForWindowCreatesMatchingRunsAndSkipsDuplicates() {
    ...
    assertThat(
            checklistRunRepository.findAll().stream().map(ChecklistRun::getDueAt).sorted().toList())
        .containsExactly(
  ...
  void generateRunsForWindowNormalizesMonthlyScheduleToLastDayOfShortMonth() {
    ...
    assertThat(checklistRunRepository.findAll())
        .singleElement()
        .satisfies(
            run -> assertThat(run.getDueAt()).isEqualTo(atOslo(dueDate, LocalTime.of(8, 30))));
  }
  ...
  void markOverdueRunsTransitionsOnlyPendingAndInProgressRuns() {
    ...
    ChecklistDefinition definition =
        createDefinition(organization, establishment, actor, "Overdue routine", null);
    ...
    int updatedRuns =
        checklistSchedulerService.markOverdueRuns(
            organization.getId(), establishment.getId(), now, currentUser);

    assertThat(updatedRuns).isEqualTo(2);
    List<ChecklistRun> persistedRuns =
        checklistRunRepository.findAll().stream()
    ...
    assertThat(findStatus(persistedRuns, inProgressRun.getId()))
        .isEqualTo(ChecklistRunStatus.OVERDUE);
    assertThat(findStatus(persistedRuns, overdueRun.getId())).isEqualTo(ChecklistRunStatus.OVERDUE);
    assertThat(findStatus(persistedRuns, completedRun.getId()))
        .isEqualTo(ChecklistRunStatus.COMPLETED);
