...
public class EstablishmentService {
  ...
  /**
   * Creates an establishment service backed by organization and persistence services.
   *
  ...
  public Page<Establishment> listEstablishments(
    ...
    var membership =
        organizationAccessService.getActiveMembershipOrThrow(currentUser, organizationId);
    if (membership.isAccessAllEstablishments()
        || membership.getRole() == org.kontrolla.organizations.domain.OrganizationRole.ORG_OWNER
        || membership.getRole() == org.kontrolla.organizations.domain.OrganizationRole.ORG_ADMIN) {
      return establishmentRepository.findByOrganizationId(organizationId, pageable);
    }

    return establishmentRepository.findAccessibleByOrganizationIdAndUserId(
        organizationId, currentUser.userId(), pageable);
  }

  /**
   * Returns a single establishment after verifying organization and establishment access.
   *
   * @param organizationId the organization identifier
  ...
  /**
   * Returns serving hours for every weekday for an establishment, defaulting missing days to
   * closed.
   *
   * @param organizationId the organization identifier
   * @param establishmentId the establishment identifier
  ...
  public java.util.List<ServingHoursDayView> getServingHours(
    ...
    var configuredHours =
        servingHoursRepository.findByEstablishmentIdOrderByDayOfWeekAsc(establishmentId).stream()
            .collect(
                java.util.stream.Collectors.toMap(
                    EstablishmentServingHours::getDayOfWeek, ServingHoursDayView::from));

    return java.util.Arrays.stream(DayOfWeek.values())
        .map(
            dayOfWeek ->
                configuredHours.getOrDefault(dayOfWeek, ServingHoursDayView.closed(dayOfWeek)))
        .toList();
  }
  ...
  public java.util.List<ServingHoursDayView> updateServingHours(
    ...
    var existingByDay =
        servingHoursRepository.findByEstablishmentIdOrderByDayOfWeekAsc(establishmentId).stream()
            .collect(
                java.util.stream.Collectors.toMap(
                    EstablishmentServingHours::getDayOfWeek, hours -> hours));
  ...
  private void validateServingHours(java.util.List<UpdateServingHoursDayCommand> commands) {
    ...
    var commandsByDay =
        new java.util.EnumMap<DayOfWeek, UpdateServingHoursDayCommand>(DayOfWeek.class);
    for (UpdateServingHoursDayCommand command : commands) {
