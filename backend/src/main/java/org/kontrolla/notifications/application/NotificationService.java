...
/** Handles notification retrieval, read-state updates, and notification creation. */
@Service
public class NotificationService {
  ...
  public Page<Notification> listNotifications(
      CurrentUser currentUser, NotificationStatusFilter status, Pageable pageable) {
    return notificationRepository.findPageForRecipient(
        currentUser.userId(), status == NotificationStatusFilter.UNREAD, pageable);
  }
  ...
  public Notification markRead(UUID notificationId, CurrentUser currentUser) {
    Notification notification =
        notificationRepository
            .findByIdAndRecipientUserId(notificationId, currentUser.userId())
            .orElseThrow(
                () ->
  ...
  /**
   * Creates a notification when the recipient is present and differs from the actor that triggered
   * the event.
   *
  ...
  public void createNotification(CreateNotificationCommand command) {
    if (command.recipientUserId() == null
        || command.recipientUserId().equals(command.actorUserId())) {
      return;
    }
