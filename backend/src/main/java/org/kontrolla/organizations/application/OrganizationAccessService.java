...
/** Centralizes organization- and establishment-level access checks for the current user. */
@Service
public class OrganizationAccessService {

  private static final Set<OrganizationRole> MEMBER_MANAGEMENT_ROLES =
      Set.of(OrganizationRole.ORG_OWNER, OrganizationRole.ORG_ADMIN);

  private static final Set<OrganizationRole> ESTABLISHMENT_MANAGEMENT_ROLES =
      Set.of(OrganizationRole.ORG_OWNER, OrganizationRole.ORG_ADMIN, OrganizationRole.ORG_MANAGER);
  ...
  public Organization getOrganizationOrThrow(UUID organizationId) {
    return organizationRepository
        ...
        .orElseThrow(
            () ->
                new ResourceNotFoundException("organization_not_found", "Organization not found"));
  }
  ...
  public void requireMembershipManagement(CurrentUser currentUser, UUID organizationId) {
    ...
    if (!MEMBER_MANAGEMENT_ROLES.contains(membership.getRole())) {
      throw new ForbiddenException(
          "organization_membership_forbidden", "Insufficient role to manage members");
    }
  }

  /**
   * Requires that the current user can manage establishments in the organization.
   *
   * @param currentUser the authenticated user
   * @param organizationId the organization identifier
   */
  ...
  public void requireEstablishmentManagement(CurrentUser currentUser, UUID organizationId) {
    ...
    if (!ESTABLISHMENT_MANAGEMENT_ROLES.contains(membership.getRole())) {
      throw new ForbiddenException(
          "organization_establishment_forbidden", "Insufficient role to manage establishments");
    }
  }

  /**
   * Requires organization-wide operational access, either through elevated role or full
   * establishment scope.
   *
   * @param currentUser the authenticated user
   * @param organizationId the organization identifier
   */
  @Transactional(readOnly = true)
  public void requireOrganizationWideOperationalAccess(
      CurrentUser currentUser, UUID organizationId) {
    if (currentUser.isPlatformAdmin()) {
      return;
    }
   ...
   */
  @Transactional(readOnly = true)
  public void requireEstablishmentAccess(
      CurrentUser currentUser, UUID organizationId, UUID establishmentId) {
    establishmentRepository
        .findByIdAndOrganizationId(establishmentId, organizationId)
        .orElseThrow(
            () ->
   ...
   */
  @Transactional(readOnly = true)
  public OrganizationMembership getActiveMembershipOrThrow(
      CurrentUser currentUser, UUID organizationId) {
  ...
  private OrganizationMembership getMembershipOrThrow(
      CurrentUser currentUser, UUID organizationId) {
    return membershipRepository
        .findByOrganizationIdAndUserId(organizationId, currentUser.userId())
        .orElseThrow(
            () ->
                new ForbiddenException("organization_access_denied", "Organization access denied"));
  }
}
