...
public interface TemperatureUnitRepository extends JpaRepository<TemperatureUnit, UUID> {
  ...
  /**
   * Returns temperature units for an establishment ordered by name with associated data loaded
   * eagerly.
   *
   ...
   */
  @EntityGraph(attributePaths = {"organization", "establishment", "logs", "logs.loggedByUser"})
  List<TemperatureUnit> findByEstablishmentIdAndOrganizationIdOrderByNameAsc(
      UUID establishmentId, UUID organizationId);
   ...
   */
  @EntityGraph(attributePaths = {"organization", "establishment"})
