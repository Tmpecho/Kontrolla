...
/** REST API for deviation listing, reporting, assignment, and timeline management. */
@RestController
...
public class DeviationController {
  ...
  public PageResponse<DeviationResponse> listEstablishmentDeviations(
      ...
      @PageableDefault(size = 20, sort = "createdAt", direction = Sort.Direction.DESC)
          Pageable pageable) {
    return PageResponse.from(
        deviationService.listDeviationsByEstablishmentId(
            organizationId, establishmentId, currentUser, pageable),
        DeviationResponse::from);
  }
  ...
  public DeviationDetailsResponse assignDeviation(
    ...
    return DeviationDetailsResponse.from(
        deviationService.assignDeviation(
            organizationId, establishmentId, deviationId, request.assignedUserId(), currentUser));
  }
  ...
  public DeviationDetailsResponse updateDeviationStatus(
    ...
    return DeviationDetailsResponse.from(
        deviationService.updateDeviationStatus(
            organizationId, establishmentId, deviationId, request.status(), currentUser));
  }
  ...
  public DeviationDetailsResponse addTimelineNote(
    ...
    return DeviationDetailsResponse.from(
        deviationService.addTimelineNote(
            organizationId, establishmentId, deviationId, request.note(), currentUser));
  }
}
