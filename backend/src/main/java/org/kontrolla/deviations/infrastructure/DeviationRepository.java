...
/** Repository for deviation persistence and organization- or establishment-scoped queries. */
public interface DeviationRepository extends JpaRepository<Deviation, UUID> {
  /**
   * Checks whether a deviation with the same title already exists for an establishment.
   *
   ...
   */
  @EntityGraph(
      attributePaths = {"organization", "establishment", "createdByUser", "assignedToUser"})
  ...
  @EntityGraph(
      attributePaths = {"organization", "establishment", "createdByUser", "assignedToUser"})
  Page<Deviation> findByEstablishmentIdAndOrganizationId(
      UUID establishmentId, UUID organizationId, Pageable pageable);
  ...
  Optional<Deviation> findByIdAndEstablishmentIdAndOrganizationId(
