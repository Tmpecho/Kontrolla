...
public class SecurityConfig {
  ...
  SecurityFilterChain securityFilterChain(
    ...
    return http.cors(Customizer.withDefaults())
        ...
        .oauth2ResourceServer(
            oauth2 -> oauth2.jwt(jwt -> jwt.jwtAuthenticationConverter(this::toAuthentication)))
        .exceptionHandling(
            exceptions ->
                exceptions
  ...
  CorsConfigurationSource corsConfigurationSource() {
    ...
    configuration.setAllowedHeaders(
        List.of("Authorization", "Content-Type", "X-Requested-With", "X-XSRF-TOKEN"));
  ...
  private AbstractAuthenticationToken toAuthentication(Jwt jwt) {
    ...
    CurrentUser principal =
        new CurrentUser(UUID.fromString(jwt.getSubject()), jwt.getClaimAsString("email"), roles);
  ...
  private void writeProblem(
    ...
    objectMapper.writeValue(
        ...
        ApiProblemDetails.create(status, code, message, request.getRequestURI()));
  }
}
