...
/**
 * REST API for authentication, invitation acceptance, CSRF bootstrapping, and self-service account
 * endpoints.
 */
@RestController
...
public class AuthController {
  ...
  public ResponseEntity<LoginResponse> refresh(HttpServletRequest request) {
    AuthSession session =
        authService.refresh(extractRefreshCookie(request), request.getRemoteAddr());
    return ResponseEntity.ok()
        .header(HttpHeaders.SET_COOKIE, refreshCookie(session.refreshToken()).toString())
        .body(
            new LoginResponse(
  ...
  public UserResponse updateMyProfile(
    ...
    return UserResponse.from(
        accountService.updateMyProfile(currentUser, request.firstName(), request.lastName()));
  }
