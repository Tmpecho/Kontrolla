...
public class UserInviteService {
	...
	public IssuedInvite issueOrganizationInvite(User user, Organization organization) {
		...
		String exposedInviteUrl = securityProperties.getInvite().isExposeInviteUrlInResponse()
				? inviteUrl
				: null;
	...
	private UserInvite resolveActiveInvite(String rawToken, Instant now) {
		return userInviteRepository.findByTokenHash(hashToken(rawToken))
				.filter(invite -> invite.isActiveAt(now))
				.orElseThrow(() -> new UnauthorizedException("invalid_invite", "Invitation is invalid or expired"));
	}
