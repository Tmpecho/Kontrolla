...
public class UserAdministrationService {
  ...
  public UserAdministrationService(
      UserRepository userRepository, AuditRecorder auditRecorder, PasswordEncoder passwordEncoder) {
  ...
  public User createUser(
    ...
    User user =
        new User(email, firstName, lastName, passwordEncoder.encode(password), active, globalRoles);
   ...
   */
  @Transactional
  public User createInvitedUser(String email, String firstName, String lastName) {
    if (userRepository.findByEmailIgnoreCase(email).isPresent()) {
      throw new ConflictException("user_already_exists", "A user with that email already exists");
    }
    ...
    User user =
        new User(
            ...
            firstName,
            lastName,
            passwordEncoder.encode(placeholderPassword),
            false,
            Set.of());
