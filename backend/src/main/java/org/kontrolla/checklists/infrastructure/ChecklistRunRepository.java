...
public interface ChecklistRunRepository extends JpaRepository<ChecklistRun, UUID> {
  ...
  long countByEstablishmentIdAndStatusInAndDueAtBefore(
      UUID establishmentId, Collection<ChecklistRunStatus> statuses, Instant dueAt);
  ...
  List<ChecklistRun>
      findByEstablishmentIdAndDefinitionGroupIdAndStatusAndDueAtGreaterThanEqualOrderByDueAtAsc(
          UUID establishmentId, UUID definitionGroupId, ChecklistRunStatus status, Instant dueAt);
