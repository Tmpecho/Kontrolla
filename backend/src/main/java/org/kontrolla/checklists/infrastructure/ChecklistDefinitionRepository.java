...
public interface ChecklistDefinitionRepository extends JpaRepository<ChecklistDefinition, UUID> {
   ...
   */
  @EntityGraph(
      attributePaths = {"establishment", "createdByUser", "updatedByUser", "tasks", "schedules"})
  ...
  @EntityGraph(
      attributePaths = {"establishment", "createdByUser", "updatedByUser", "tasks", "schedules"})
  List<ChecklistDefinition> findByDefinitionGroupIdAndEstablishmentIdOrderByVersionNumberAsc(
      UUID definitionGroupId, UUID establishmentId);
  ...
  @EntityGraph(
      attributePaths = {"establishment", "createdByUser", "updatedByUser", "tasks", "schedules"})
  List<ChecklistDefinition> findByEstablishmentIdAndStatus(
      UUID establishmentId, ChecklistDefinitionStatus status);
}
