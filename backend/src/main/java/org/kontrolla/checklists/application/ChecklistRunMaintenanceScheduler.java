...
/** Scheduled maintenance job that generates upcoming checklist runs and marks overdue runs. */
@Component
@Profile("!test")
public class ChecklistRunMaintenanceScheduler {
  ...
  /** Performs periodic run generation and overdue maintenance for active establishments. */
  @Scheduled(
      initialDelayString = "${app.checklists.overdue.initial-delay-ms:30000}",
      fixedDelayString = "${app.checklists.overdue.fixed-delay-ms:300000}")
              ...
              .mapToInt(
                  establishment ->
                      checklistSchedulerService.generateRunsForWindowInternal(
                          establishment, generationWindowStart, generationWindowEnd, actor.getId()))
              .sum();
    }
            ...
            .mapToInt(
                establishment ->
                    checklistRunService.markOverdueRuns(
                        establishment.getOrganization().getId(), establishment.getId(), now, null))
            .sum();
