...
public class ChecklistAccessService {
  ...
  public void requireChecklistExecutionAccess(
      UUID organizationId, ChecklistRun checklistRun, CurrentUser currentUser) {
    if (canManageChecklistOperations(
        organizationId, checklistRun.getEstablishment().getId(), currentUser)) {
      return;
    }
    ...
    boolean assignedToCurrentUser =
        checklistRun.getAssignments().stream()
            .anyMatch(
                assignment -> assignment.getAssignedUser().getId().equals(currentUser.userId()));

    if (!assignedToCurrentUser) {
      throw new ForbiddenException(
          "checklist_run_execution_forbidden", "You are not assigned to this checklist run");
    }
  }

  /**
   * Indicates whether the current user can manage checklist operations for an establishment.
   *
   * @param organizationId the organization identifier
   * @param establishmentId the establishment identifier
   * @param currentUser the authenticated user
   ...
   */
  @Transactional(readOnly = true)
  public boolean canManageChecklistOperations(
      UUID organizationId, UUID establishmentId, CurrentUser currentUser) {
  ...
  public void requireAssignmentFilterAccess(
      UUID organizationId, UUID establishmentId, UUID assignedUserId, CurrentUser currentUser) {
  ...
  public ResourceNotFoundException checklistAssignmentNotFound() {
    return new ResourceNotFoundException(
        "checklist_run_assignment_not_found", "Checklist run assignment not found");
  }
}
